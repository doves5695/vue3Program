    ...
    <el-form-item label="销售属性">
      <el-form :inline="true">
        <el-form-item v-for="(item, index) in saleArr" :key="item.id" :label="item.saleAttrName">
          <el-select>
            <el-option v-for="(saleAttrVale, index) in item.spuSaleAttrValueList" :key="saleAttrVale.id" :label="saleAttrVale.saleAttrValueName"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </el-form-item>
    <el-form-item label="图片名称">
      <el-table border :data="imgArr">
        ...
        <el-table-column label="操作">
          <template #="{row, $index}">
            <el-button type="primary" size="small" icon="Plus">添加默认</el-button>      
          </template>
        </el-table-column>
      </el-table>
    </el-form-item>
